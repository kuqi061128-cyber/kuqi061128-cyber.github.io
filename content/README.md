...
> ⚠️ **2026-08 动态化后的定位：本目录现在是「兜底数据源」。**
> 日常发文请走管理后台（Content Manager），后台内容才是正式内容。
> 只有当 Strapi 后端不可用时，网站才会自动回退加载这里的文件。
> 新增兜底文章要同步两处：本目录文件 + js/api-loader.js 的 LOCAL_FILES 清单。
